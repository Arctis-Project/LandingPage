    ...
    <section id="founders" className="py-28 px-6">
      <div className="max-w-3xl mx-auto">
        <div ref={ref} className={cn("text-center mb-16", inView ? "animate-fade-up" : "opacity-0")}>
          ...
          <p className="text-[var(--text-sec)] text-lg font-light">The people behind Arctis Project</p>
        </div>

        <div className={cn("grid gap-5 sm:grid-cols-2", inView ? "stagger" : "opacity-0")}>
          {founders.map((f) => (
            <div key={f.name} className="group rounded-[28px] bg-[var(--bg-elevated)] border border-[var(--border-c)]/50 p-8 text-center transition-all duration-300 hover:shadow-md hover:border-[var(--border-c)]">
              <div className="mb-5 flex justify-center">
                <div className="flex h-16 w-16 items-center justify-center rounded-[20px] bg-[var(--md-secondary-container)] text-[var(--md-on-secondary-container)] transition-all duration-200 group-hover:bg-[var(--md-primary)] group-hover:text-[var(--md-on-primary)]">
